...
func (r *Propagator) getPolicyPlacementDecisions(
	...
	case "PlacementRule":
		...
		if plr.Name == "" { // NOTE: for debugging
			plr.Name = "not-found"
		}

		for i := range placements {
			placements[i].PlacementRule = plr.Name // will be "not-found" if the PlacementRule was not found
		}
	case "Placement":
		...
		if pl.Name == "" { // NOTE: for debugging
			pl.Name = "not-found"
		}

		for i := range placements {
			placements[i].Placement = pl.Name // will be "not-found" if the Placement was not found
		}
	}
...
func (r *Propagator) getAllClusterDecisions(instance *policiesv1.Policy, pbList *policiesv1.PlacementBindingList) (
...
func (r *Propagator) handleDecisions(instance *policiesv1.Policy) ([]*policiesv1.Placement, error) {
	...
	// Add any existing replicated policies, because they need to remove themselves.
	// Note: this was added for the case where the placementbinding is changed, maybe that should
	// have a separate reconciler?
	policyList := &policiesv1.PolicyList{}
	opts := &client.ListOptions{}

	matcher := client.MatchingLabels{common.RootPolicyLabel: common.FullNameForPolicy(instance)}
	matcher.ApplyToList(opts)

	// Save this error for later, some work should still be accomplished now
	var savedErr error

	if err = r.List(context.TODO(), policyList, opts); err != nil && !k8serrors.IsNotFound(err) {
		savedErr = err
	}

	for _, replicated := range policyList.Items {
		dec := appsv1.PlacementDecision{
			ClusterName:      replicated.Namespace,
			ClusterNamespace: replicated.Namespace,
		}

		if _, found := allClusterDecisions[dec]; !found {
			allClusterDecisions[dec] = policiesv1.BindingOverrides{}
		}
	}
...
func (r *Propagator) handleRootPolicy(instance *policiesv1.Policy) error {
	...
	_, err := r.handleDecisions(instance)
	if err != nil {
	...
	/* Is this status update necessary?
